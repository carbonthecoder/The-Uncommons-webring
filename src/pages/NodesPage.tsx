...
export const NodesPage: React.FC = () => {
  ...
  const [simulatingSlot, setSimulatingSlot] = useState<string | null>(null);
  ...
  return (
    <div className="space-y-8 w-full max-w-6xl mx-auto px-2 sm:px-4">
                ...
                >
                  ...
                  <div className="col-span-2 flex items-center justify-end gap-2 w-full lg:w-auto pt-2 lg:pt-0 border-t border-white/[0.04] lg:border-0">
                    <button
                      onClick={() => {
                        sound.playClick();
                        setSimulatingSlot(item.id);
                        setSimDomain('');
                        setSimName('');
                        setSimField('');
                      }}
                      className="px-2.5 py-1 bg-zinc-900 hover:bg-zinc-800 border border-white/10 rounded text-zinc-400 hover:text-white transition-colors cursor-pointer text-[11px] flex items-center gap-1"
                    >
      ...
      {/* ========================================================================= */}
      {/* MODAL 1: INTERACTIVE ENTRY SIMULATOR ("Test-Drive Your Slot") */}
      {/* ========================================================================= */}
      {simulatingSlot && (
        <div
          ...
          onClick={() => setSimulatingSlot(null)}
        >
          <div
            className="relative w-full max-w-xl bg-zinc-950 border border-white/20 rounded-2xl p-6 shadow-2xl space-y-5"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="flex items-center justify-between pb-3 border-b border-white/[0.08]">
              <div className="flex items-center gap-2 text-xs font-mono text-amber-400">
                <Sparkles className="w-4 h-4" />
                <span>ENTRY SIMULATOR // {simulatingSlot}</span>
              </div>
              <button
                onClick={() => setSimulatingSlot(null)}
            ...
            <div className="space-y-1">
              <h3 className="font-syne text-lg font-bold text-white">
                Test-Drive Your Slot Entry
              </h3>
              <p className="text-xs font-sans text-zinc-400">
                See exactly how your sovereign domain will look inside the official Uncommons Ledger before submitting to council review.
              </p>
            </div>
            ...
            <div className="space-y-3 font-mono text-xs">
              <div>
                <label className="block text-zinc-400 mb-1">Your Sovereign Domain</label>
                <input
                  type="text"
                  value={simDomain}
                  onChange={(e) => setSimDomain(e.target.value)}
                  placeholder="e.g. alex.dev"
                  className="w-full px-3 py-2 bg-black border border-white/10 rounded text-white placeholder-zinc-600 focus:outline-none focus:border-white/30"
                />
              </div>

              <div>
                <label className="block text-zinc-400 mb-1">Your Name / Handle</label>
                <input
                  type="text"
                  value={simName}
              ...
              <div>
                <label className="block text-zinc-400 mb-1">Primary Field / Craft</label>
                <input
                  type="text"
                  value={simField}
                  onChange={(e) => setSimField(e.target.value)}
                  placeholder="e.g. Systems Compilers & AI Orchestration"
                  className="w-full px-3 py-2 bg-black border border-white/10 rounded text-white placeholder-zinc-600 focus:outline-none focus:border-white/30"
                />
              </div>
            </div>
            ...
            {/* Live Row Preview */}
            <div className="space-y-1">
              <span className="text-[10px] font-mono text-zinc-500 uppercase tracking-wider">LIVE LEDGER PREVIEW</span>
              <div className="p-3 bg-zinc-900 border border-emerald-500/30 rounded-lg text-xs font-mono space-y-1">
                <div className="flex items-center justify-between text-[10px]">
                  <span className="px-1.5 py-0.5 rounded bg-emerald-950 text-emerald-400 border border-emerald-500/20 font-semibold">
                    {simulatingSlot}
                  </span>
                ...
                <div className="font-bold text-white">
                  {simDomain.trim() || 'your-domain.dev'}
                </div>
                <div className="text-zinc-400 font-sans text-xs">
                  {simName.trim() || 'Your Name'} &bull; {simField.trim() || 'Your Obsession Field'}
                </div>
              </div>
            </div>
            ...
            <div className="pt-3 border-t border-white/[0.08] flex items-center justify-between gap-3">
              ...
              <Link
                to="/apply"
                onClick={() => sound.playClick()}
                className="px-4 py-2 bg-emerald-400 hover:bg-emerald-300 text-zinc-950 font-mono text-xs font-bold rounded-md transition-all flex items-center gap-1.5 cursor-pointer shadow"
              >
                <span>Ready? Apply in #council-review</span>
                <ArrowRight className="w-3.5 h-3.5" />
              </Link>
            </div>
          </div>
